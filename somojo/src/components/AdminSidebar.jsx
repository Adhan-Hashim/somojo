...
export default function AdminSidebar({ activeTab, onTabChange }) {
    ...
    return (
        <div className="w-64 h-screen bg-slate-900/60 backdrop-blur-2xl border-r border-white/5 flex flex-col fixed left-0 top-0 z-[60] shadow-2xl">
            {/* Logo */}
            <div className="p-8">
                <div className="flex items-center gap-4 group cursor-pointer">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-violet-500 flex items-center justify-center font-bold text-white shadow-lg group-hover:scale-105 transition-transform">
                        S
                    </div>
                    <div>
                        <h1 className="text-xl font-bold tracking-tight text-white">Somojo</h1>
                        <p className="text-[11px] text-indigo-400 font-medium uppercase tracking-wider">Admin Workspace</p>
                    </div>
                </div>
            </div>
                    ...
                    return (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-all duration-300 relative overflow-hidden ${isActive
                                    ? "text-indigo-300 bg-indigo-500/10 shadow-[0_4px_20px_-4px_rgba(99,102,241,0.2)]"
            ...
            <div className="p-6 border-t border-white/5">
                <div className="bg-slate-800/40 rounded-2xl p-4 mb-4 border border-white/5">
                    <div className="flex items-center justify-between mb-2">
                        <span className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">Status Node</span>
                        <div className="w-2 h-2 bg-emerald-400 rounded-full animate-pulse shadow-[0_0_8px_rgba(52,211,153,0.5)]" />
                    </div>
                    <p className="text-xs text-slate-300 font-medium tracking-wide">Operational Logs OK</p>
                </div>
